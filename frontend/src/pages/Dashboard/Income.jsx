...
export const Income = () => {
  ...
  const addIncomeDetails = async (income) => {
    ...
    try {
      ...
      console.error("Add income error:", error?.response?.data || error.message || error);
    }
  };
  const deleteIncomeDetails = async () => {
    if (!openDeleteAlert.data) return;
    try {
      await axiosInstance.delete(API_PATHS.INCOME.DELETE_INCOME(openDeleteAlert.data));
  ...
  useEffect(() => {
    fetchIncomeDetails(); // auto-fetch data when page loads
  }, []);
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="">
            ...
            >
              ...
              <AddIncomeForm onAddIncome={addIncomeDetails}/>
            </Modals>
          </div>
        </div>
      </div>
